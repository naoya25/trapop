...
- 起動済みのアプリに `trapop://new` が届いたら: パネルが隠れていれば再表示(現在の Space へ移動+再固定)、無ければ生成。メイン窓の表示状態は変えない
...
4. 赤ボタン/Esc でパネルだけ消え、アプリは継続。再度 `trapop://new` で即再表示
5. パネルでの翻訳がメイン窓の履歴一覧に出る
6. 通常起動ではメイン窓が出て、従来の全機能(履歴・設定含む)が動く
7. Dock クリックでメイン窓が表示される
